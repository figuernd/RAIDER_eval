...
main(int argc, char * argv [])
{
  ...
  errflg=0;
   ...
   Max_rep=2.;
   zero=0;
  while  (! errflg && ((ch = getopt (argc, argv, "g:r:")) != EOF))
    switch  (ch)
        {
	 ...
	 if  (p == optarg )
              {
               fprintf (stderr, "ERROR:  Illegal min gap length \"%s\"\n",
                        optarg);
               exit (-1);
              }
              optarg = NULL;
	      break;
           case 'r' :
         Min_leng=(int) strtol (optarg, & p, 10);
	 if  (p == optarg )
              {
               fprintf (stderr, "ERROR1:  Illegal min repeat length\"%s\"\n",
                        optarg);
               exit (-1);
              }
  ...
  while (fscanf(fp,"%d %d %d %d \n",&cr1,&cr2,&leng2,&num2) != EOF) {
    ...
    if ( (diff1 < leng1  || diff1+leng2< leng1 ) &&(cr1 !=fr2) &&((num1 !=num2) &&((cr2<cr1 &&cr2<classr1 && cr2>0 && !fsign) ||(classr1_leng+classr1>cr1+leng2 && !fsign)||(cr2<0 && !fsign)||(cr2>cr1 && !fsign)||(cr1 -classr1 +1<Min_leng && !fsign) || (fsign))) &&(Cur_fbac1==Cur_cbacsof)){
	...
	if (merge){
	 ...
	 subs=subs+1;}
      ...
      delta=(double)(diff1-leng1)/(leng1+leng2);
       Idelta=(diff1-leng1);

       if ( (Idelta<Igap)&&fr1 &&  (delta<Max_rep)&& (cr1 !=fr2) && ((num1 !=num2)&&((cr2<cr1 &&cr2<classr1&&cr2>0&& !fsign) ||(cr2>cr1 && !fsign) ||(cr2<0 && !fsign) ) || (fsign)) && (Cur_fbac1==Cur_cbacsof)) {
	 if (fsign ||subs){
	   if (subs){
	     long_leng=res_leng;
	     merge=subs;
	     ...
	     ci1=0;csign1=0; csign2=0;
             if (Rlist[Cur_rep1_cl].clname==0){
	       Rlist[Cur_rep1_cl].clname=e_num;
               e_cur=e_num;
	     }
             else {e_cur= Rlist[Cur_rep1_cl].clname;
	     addInName(e_cur,classr1);
	 ...
	 long_temp=long_leng-leng1+diff1+leng2;
	 if (classr1 + long_temp <classr2){
	 ...
	 num2=num1;
	 merge=merge+1;
	 /*printf ("%f %d %d %d\n",delta, long_leng,diff1-leng1, leng2);*/
	 leng2=long_leng;
	 ...
	 if(long_leng){
	   ...
	   /*fprintf(f2, "here2 %10d %10d %10d %10d\n",classr1,classr2, long_leng, merge);*/
	   /* fprintf(stderr, "here2 %10d %10d %10d %10d\n",classr1,classr2, long_leng, merge); */
	
	   
	   Elist[e_num].length=long_leng;
	   Elist[e_num].copies=merge;
	 ...
	 if (res_leng){
	   if (cr2>classr1 && classr1 + res_leng> cr1 && cr2<cr1 && cr1 -classr1 +1>=Min_leng){
	...
	if (Rlist[Cur_rep1].clname!=e_num){
	  ...
	  addInName(e_cur,classr1);
	     ...
	     /*fprintf(f2, "here4 %10d %10d %10d %10d \n",classr1,cur_cr2,zero,zero);*/
	     /*fprintf(stderr, "here4 %10d %10d %10d %10d \n",classr1,cur_cr2,zero,zero);*/
  ...
  if(long_leng){
    ...
    /* fprintf(f2, "here8 %10d %10d %10d %10d\n",classr1,classr2, long_leng,merge);*/
    
    /*fprintf(stderr, "here8 %10d %10d %10d %10d\n",classr1,classr2, long_leng,merge);  */
