...
seqan::ArgumentParser::ParseResult parseCommandLine(AppOptions & options, int argc, char const ** argv) {
	...
	addDescription(
			parser,
			"RAIDER parses the given sequence file using the supplied mask (spaced seed) to identify de novo repeats. Minimum repeat size and other options can be configured as described below.");
	...
	addOption(
			parser,
			seqan::ArgParseOption("c", "count", "Minimum number of repeats in a family. Defaults to 8.",
					seqan::ArgParseOption::INTEGER));
...
void printArgs(AppOptions &options) {
	if (options.verbosity > 0) {
		cout << "__ARGUMENTS____________________________________________________________________" << endl
				...
				<< "OUTPUT_DIR\t" << options.output_directory << endl;
	}
}
...
// Load sequences into master sequence, mark where each sequence ID begins within the master
	...
	// sequence is master sequence (all sequences in file concatenated)
	// thresholds: indices in master seq paired w seqIds marking start of each indiv. seq
