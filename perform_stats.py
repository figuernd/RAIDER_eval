...
def get_stats(real_indices, gen_indices, gen_length):
    ...
    while len(real_indices) > 0 and len(gen_indices) > 0:
        s1, e1 = [real_indices[0][i] for i in range(2)]   # KARRO: This would be better done as: s1,e1 = real_index[0][:2]
...
def main(seq_file, real_repeats, masker_output, output_file, print_reps):
    ...
    l = len(orig_record.seq)
    
    real_indices = get_repeats(real_repeats)
    gen_indices = get_repeats(masker_output)
    tp, fp, fn, tn, tpr, tnr, ppv, npv, fpr, fdr, tps, fps, fns= get_stats(real_indices, gen_indices, l)
...
if __name__ == "__main__":
    ...
    #parser.add_argument("sim_seq", help = "Simulated Sequence file")
    ...
    main(args.seq_file, args.repeat_file, args.masker_output, args.output_file, args.print_reps)
    #main(args.seq_file, args.repeat_file, args.sim_seq, args.masker_output, args.output_file, args.print_reps)
