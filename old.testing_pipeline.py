...
def raider_pipeline(raider_exe, input_file, seed, f):
    ...
    if args.run_blast:
        ...
        title4 = "bl." + title
        p4 = launch_job(cmd=cmd4, title=title4, base_dir=elements_dir, modules=Locations['blast_modules'], depend=[p2], attrs={'blast_output':blast_output}, ppn = args.num_threads, walltime = blast_walltime_default)
    ...
    if args.rs_filters >= 2:
        ...
        if args.run_blast:
            cmd10 = blast_cmd.format(blast = Locations['blast'], blast_format = blast_format, output = blast_output, consensus_file = filter2_output, db_file = database_file,
                                     evalue = args.evalue, short = "-task blastn-short" if args.short else "", max_target = args.max_target, num_threads = args.num_threads)
            title10 = "bl2." + title 
            p10 = launch_job(cmd=cmd10, title=title10, base_dir=output2__dir, modules=Locations['blast_modules'], depend=[p8], ppn = args.num_threads, walltime = blast_walltime_defalt)
        else:
            p10 = None
