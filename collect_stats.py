...
def collectRaider(DIR, tool):
    ...
    for org in data_map.keys():
        for seed_num in seed_map.keys():
            for f in f_list):
    for file in glob.glob(DIR + "/*.RM/*.fa.out"):
        r = regex.search(file)
        ...
        job_file = DIR + "/../job_log/{prefix}.{org}.{seed_num}.{f}".format(prefix = tool_prefix[tool], org=org, seed_num=seed_num, f=f)        
        ...
        job_file = DIR + "/../job_log/rm.{prefix}.{org}.{seed_num}.{f}".format(prefix = tool_prefix[tool], org=org, seed_num=seed_num, f=f)        
        if os.path.exists(job_file):
            p = redhawk.loadPBS(open(job_file, "rb"))[0]
            try:
                if p.efile_exists():
        ...
        blast_output = "{DIR}/{org}.s{seed}.f{f}.blast.6.txt".format(DIR=DIR, org=org, seed=seed_num, f=f)
        if os.path.exists(blast_output):
            output = "{DIR}/{org}.s{seed}.f{f}.pra.txt".format(DIR=DIR, org=org, seed=seed_num, f=f)
