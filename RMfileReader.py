...
def createObjArrays(alignFilePointer, outFilePtr1):
...
def createRepObjs(outObjs, alignObjs):
...
def pickleRepeats(repeatList, fileName, compressed = False):
    """
    ...
    """
    fp = bz2.BZ2File(fileName + ".bz2", 'w') if compressed else open (fileName, "w")
    for obj in repeatList:
        cPickle.dump(obj, fp)
    fp.close()

def unpickleRepeats(fileName):
    """Will unpickle the .bz2 version if:
    1) The name ends with .bz2.
    2) No non-compressed erion is present.
    """
    if fileName.endswith(".bz2"):
        fp = bz2.BZ2File(fileName + ".bz2", 'r')
    else:
        try:
            fp = open(fileName, 'r')
        except:
